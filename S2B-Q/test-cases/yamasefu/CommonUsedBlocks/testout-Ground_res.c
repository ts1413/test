...
/* IN_0001 */
pthread_mutex_t mutex__IN_0001 = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t cond__IN_0001 = PTHREAD_COND_INITIALIZER;
int flag__IN_0001 = 0;
...
void mcos_lcore_task_0001(uint32_t stacd, uintptr_t exinf);
...
static void *f__mcos_lcore_task_0001 (void *arg)
{
  if (PTHREAD_DEBUG) {
    fputs ("start mcos_lcore_task_0001\n", stdout);
    fflush (stdout);
  }
  mcos_lcore_task_0001 (0, (uintptr_t)arg);
  return NULL;
}

...
#include "testout-Demux_res.c"
#include "testout-Demux.h"
...
{  /* omit input channel bit vector */
  ...
  while (1) {
	...
	memcpy(CH_0000_0100.Demux_In1_1,
	  Demux_In1_1,
	  sizeof Demux_In1_1);
	CH__SYNCM();
...
{  /* omit output channel bit vector */
  ...
  while (1) {
	...
	memcpy(CH_IO0001_0000.Demux_In1_1,
	  Demux_In1_1,
	  sizeof Demux_In1_1);
	CH__SYNCM();
